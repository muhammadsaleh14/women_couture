  ...
  return (
    <div className="flex min-h-screen flex-col bg-stone-50">
      <header className="sticky top-0 z-40 border-b border-stone-200/80 bg-stone-50/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-lg items-center justify-between px-4 sm:max-w-3xl lg:max-w-5xl">
          <Link
            to={ROUTES.home}
          ...
          <Button variant="ghost" size="sm" asChild className="gap-1.5">
            <Link to={ROUTES.cart}>
              ...
              <span className="tabular-nums">Cart{count > 0 ? ` (${count})` : ""}</span>
            </Link>
          </Button>
        </div>
        <Separator />
